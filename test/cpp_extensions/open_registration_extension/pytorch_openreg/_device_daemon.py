...
class Driver:
    ...
    def destroyEvent(self, event, device_index):
        self.run_on_exeuctor(device_index, "eventDestroy", event)
        self.event_belong.pop(event)

    @register(registry)
    def synchronizeEvent(self, event):
        self.run_on_exeuctor(self.event_belong[event], "eventSynchronize", event)

    @register(registry)
    def queryEvent(self, event):
        return self.run_on_exeuctor(self.event_belong[event], "eventQuery", event)

    @register(registry)
    def elapsedTime(self, e1, e2, device_index):
        return self.run_on_exeuctor(device_index, "eventElapsedTime", e1, e2)

    @register(registry)
    def block(self, event, stream):
        self.run_on_exeuctor(stream.device_index, "block", event, stream.stream_id)
